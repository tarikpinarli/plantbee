...
func (h *Handler) HandleAddPlant(w http.ResponseWriter, r *http.Request) {
	...
	if req.ImageURL != "" &&
		!strings.HasPrefix(req.ImageURL, "http://") &&
		!strings.HasPrefix(req.ImageURL, "https://") &&
		!strings.HasPrefix(req.ImageURL, "/uploads/") {
		jsonError(w, "Image URL must be a valid link or local upload path", http.StatusBadRequest)
		return
	}
